...
public class UpdateUserServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			int executeUpdate = statement.executeUpdate("update user set password='" + parameter2 + "'where email='" + parameter1
					+ "'");
			PrintWriter writer = response.getWriter();
			if (executeUpdate != 0) {
				writer.println("<H1>USER UPDATED</H1>");
			} else {
				writer.println("<H1>NOT UPDATED</H1>");
			}
		} catch (SQLException e) {
			e.printStackTrace();
		}
